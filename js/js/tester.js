console.log("tester.js er lastet - SUPERRETTET 23:45");

var innloggetBruker = window.innloggetBruker || null;
var erAdmin = window.erAdmin || false;

async function hentInnloggetBrukerTilTest() {
  if (innloggetBruker) return innloggetBruker;

  if (typeof supabaseClient !== "undefined" && supabaseClient.auth) {
    const { data, error } = await supabaseClient.auth.getUser();
    if (!error && data && data.user) {
      innloggetBruker = data.user;
      window.innloggetBruker = data.user;
      return data.user;
    }
  }
...
async function testRettigheter(){
  ...
  try{
    ...
    const bruker = await hentInnloggetBrukerTilTest();
    erAdmin = Boolean(window.erAdmin || erAdmin);
    ...
    leggTilMelding("testMelding","erAdmin = "+erAdmin,true);
